...
class WebpackOptionsApply extends OptionsApply {
	...
	process(options, compiler) {
		// 1. process方法的作用：将传入的options转成webpack的plugin注入到webpack生命周期中
		// 下面的代码都是将内置的Plugin进行导入(所以plugin事实上贯穿webpack的整个构建流程)
		// 接下来的各种操作,就是new InnerPlugin().apply()的调用过程
		// 这些Plugin后续是通过tapable来实现钩子的监听, 并进行自己的处理逻辑

		// 插件调用apply()注册的目的就是为了传入compiler对象，插件通过compiler执行各种
		// 逻辑，如监听不同的hook，在webpack生命周期的对应阶段执行插件本身想做的操作
